...
def fitFunc(x):  # 適應度函數
    return (-20 * math.exp(-0.2 * math.sqrt( 1 / NUM_BIT * sum(x[i] * x[i] for i in range(NUM_BIT)) - math.exp( 1 / NUM_BIT * sum(math.cos(2 * math.pi * x[i]) for i in range(NUM_BIT)))+ 20+ math.exp(1))))
